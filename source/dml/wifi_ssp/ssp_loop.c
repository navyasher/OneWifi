    ...
    if (ret == RETURN_OK) {
        ...
        if (strlen(mcfg->mac) != 0) {
            ...
            if (ret == RETURN_OK) {
                ...
                if (hash_map_put(psm_mac_map, mcfg_mac, temp_mac_entry) == -1) {
                    wifi_util_error_print(WIFI_PSM, "%s:%d:hash_map_put failed\n", __func__, __LINE__);
                    return;
                }
        ...
        if (strlen(mcfg->mac) != 0) {
            ...
            if (ret == RETURN_OK) {
                ...
                if (hash_map_put(psm_mac_map, strdup(mcfg->mac), temp_mac_entry) == -1) {
                    wifi_util_error_print(WIFI_PSM, "%s:%d:hash_map_put failed\n", __func__, __LINE__);
                    return;
                }
                count = hash_map_count(psm_mac_map);
